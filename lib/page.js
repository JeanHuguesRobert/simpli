...
// "Code" pages first user is the name of the user that logs in.
  ...
  if( !users )return ""
  // Get rid of Curator qualifier in code pages
  if( this.isCode() ){
    users = users.replace( /Curator/g, "")
  }
...
PageProto.isCuratorCode = function(){
  if( !this.isCode() )return false
  return this.getUsers().includes( "Curator")
}
PageProto.declareIdempotentPredicate( "isCuratorCode")
...
// A "Guest" page is a page that is about a non member user typically
