...
// Look for or make a Page object to reference said page
...
// Deleted pages's content is moved to archive
  ...
  var that = this;
  this.copyPage( page, "Copy" + page.name, function( err1 ){
    that.copyPage( page, mvname, function( err ){
      if( err1 || err ) return cb.call( that, err1 || err );
...
Session.mayEdit = function( page, mayread ){
  ...
  // This includes secrets, copies, deletions, etc...
  // Why: I'm not sure...
  if( page.inherited
  &&  page.isSpecial()
  &&  page.name != "AboutWiki"
  ...
  // c2 wiki is read only
  if( this.wiki.name === "c2" )return false;
  ...
  // PrivateXXX pages are protected too.
  if( ( page.isCopy() || page.isDeletion() || page.isPrivate() )
  && !this.canCurator
  ){
    return false;
  }
