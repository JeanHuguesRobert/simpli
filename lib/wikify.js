...
// ToDo: nice icons, unintrusive http://camendesign.com/code/uth2_css3-hyperlinks
  ...
  .replace( /\(\?\?\?([\s\S]*?)\?\?\?\)/g,
    '<div class="sw_what"><span class="glyphicon glyphicon-info-sign"></span><div class="sw_what_content">$1</div></div>')
  // "xxxxx"
  .replace(    /([^=]|^)"([^"><]{2,}?[^="\s><])"/g,
    "$1&quot;<cite>$2</cite>&quot;")
  // **xxxxx**
  ...
  if( this.isIe6 || this.isIe7 ){
    text = text
    .replace( /\n/g, "<br>\n")
    .replace( /<span class="glyphicon glyphicon-info-sign-in"><\/span>/g, "")
  }
  if( !this.canScript ){
    text = text
...
// Process basic markup rendering
  ...
  && this.wiki.name != "c2"
    ...
    for( item in this.wiki.allPages ){
      ...
      if( !"Do".starts(   item)
      &&  !"#rfc".starts( item)
      &&  (!"UserRestore".starts( item) || this.isCurator)
      &&  this.mayRead( itempage)
      &&  (!(itempage.wasIncarned() && itempage.isVoid()) || this.isCurator)
  ...
  if( is_code || is_rfc ){
    text = text.replace( /(\W)(RFC)([ #]*)(\d+)/gm,
      '$1<a href="' + href + 'tag_rfc$4">$2$3$4</a>'
    )
  }
  
  // Kudo zzzz @xxxx become vote buttons
  if( SW.kudocracy ){
    text = text.replace( 
  ...
  // JHR: 2014. That was fun, but premium will work differently, if ever
  if( false
  && !this.wiki.isRoot()
