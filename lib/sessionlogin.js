...
// July 2025 by @jhr, SimpliKudo integration with GitHub OAuth
...
// Note: that name does not include @, it is added here.
// Called by Session.login()
...
// It is called by either twitterLoginForm() or ...
  ...
  var that = this;
  this.login_de&&bug( 
    "userLogin:", user, "curator:", is_curator, "page:", page
  );
  ...
  // Note: this is no longer true with ?kudocracy= style (dec 2014)
  ...
  if( is_very_open_by_default ){
    this.login_de&&bug(
      "identified users are curators of still very open wiki"
    );
    is_curator = true;
  ...
  if( user !== "SomeGuest" ){
    // Avoid that in Kudocracy, disturbing for visitor who does not need to
    // Know about the double login (first in Kudocracy, then in SimpliWiki)
    if( !this.isKudocracy ){
  ...
  userpage.read( function( err ){
    if( userpage.isNull() ){
      ...
      && !is_curator
        ...
        that.wiki_de&&bug( 
          "newAuthenticUser: ", user, ", becomes member of wiki"
        );
    ...
    // If page exists, extract user name from first line of text, if any
      ...
      if( user != name ){
        ...
        // Avoid disturbing msg in Kudocracy, to hide double login
        if( !that.isKudocracy ){
...
// It is WAY too complex.
  ...
  // OK, now let's find the "User" page, default content is provided
  ...
  if( has_curator ){
    ...
    that.login_de&&bug( 
      "Will be Curator:", is_curator ? "curator" : "not curator"
    );
  }
  ...
  that.login_de&&bug( 
    "desiredPage:", page, "currentPage:", that.getCurrentPage()
  );
...
// Return false if nothing happened, true if req is beeing processed.
  ...
  if( query && query.kudocracy ){
    // Use SimpliKudo module to parse context
    var kudo_ctx = Kudo.parseKudoContext( query.kudocracy );
    if( kudo_ctx ){
      this.canScript = !!kudo_ctx.can_script;
      this.isKudocracy = kudo_ctx;
      // As an iframe or not
      if( query.iframe === "true" ){
        this.isIframe = this.httpLogin.iframe = true;
      }
      this.config.lang = kudo_ctx.lang;
      var kudo_name = kudo_ctx.authentic && kudo_ctx.visitor && kudo_ctx.visitor.substring( 1 );
      // Login
      if( kudo_name ){
        new_name = kudo_name;
      }else{
...
//
// 2026: Now also supports GitHub OAuth cookies

  // Try GitHub cookies first (preferred)
  var github_name = this.getCookie( "sw_github_name", req);
  var github_id = this.getCookie( "sw_github_id", req);
  var github_screenname = this.getCookie( "sw_github_screenname", req);

  if( github_name && github_id && github_screenname ){
    // Sanitize
    github_id = github_id.replace( /[^A-Z_a-z0-9]/g, "");
    if( github_name !== "null" && github_name !== "SomeGuest" ){
      this.twitterId   = this.cookiedTwitterId   = github_id;
      this.twitterName = this.cookiedTwitterName = github_name.replace( "@", "" ); // Remove @ for internal use
      this.login_de&&bug( "GitHub cookie:", this.twitterName);
      // SimpliKudo: read configurable Kudo Twitter name
      var kudo_twitter_name = this.getCookie( "sw_kudo_twitter_name", req);
      if( kudo_twitter_name ){
        this.kudoTwitterName = kudo_twitter_name;
        this.login_de&&bug( "Kudo Twitter name:", this.kudoTwitterName);
      }
      return this.twitterName;
    }
  }

  // Fallback to Twitter cookies for compatibility
...
// It tracks the visit in the user's list of visited wikis.
  ...
  if( !this.user ){
    ...
    // Don't do that in Kudocracy, double login is disturbing
    if( !this.isKudocracy ){
