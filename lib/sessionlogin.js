...
// parent wiki before being logged in the child wiki.
  ...
  // If no valid code was provided, figure out something
    ...
    if( !code ){
      ...
      if( code ){
        // If "secret" code, transform it somehow so that curator cannot
...
// It is called by either twitterLoginForm() or ...
  ...
  if( this.loginWithCode ){
    ...
    is_curator || (is_curator = this.canCurator)
  ...
  userpage.read( function( err ){
    ...
    // More work
    that.loginForm( user, code, false, is_curator, page, req)
  })
}
...
// Until this is done, I get back to literate programming
  ...
  // Whatever happens, twitter user is curator of her own wiki
  var is_curator = false
  if( this.fixTwitterLogin( null, req) ){
    this.login_de&&bug( "also twitter owner is curator")
    is_curator = true
  }
  ...
  if( page.isNull() ){
    ...
    if( page.isDisposableCode() ){
      ...
      // Some help
      var init = user
      if( !this.wiki.isVeryOpen() && !is_curator ){
      ...
      // If own wiki or very open but just created, user will be a curator
      if( is_curator || this.wiki.isVeryOpenByDefault() ){
        if( !is_curator ){
          this.login_de&&bug( "open by default, make curator, user:", user)
          is_curator = true
        }
        this.putPage( page, "Curator" + init, function( err ){
          // ToDo: err handling
        })
      }else{
  ...
  this.loginForm( user, code, intentional_code, is_curator, page, req)
}
...
Session.loginForm = function( user, code, intentional, has_curator, page, req ){
...
// It is WAY too complex.
  ...
  var is_config_defined_curator = false
  if( this.wiki.config.curatorCode
  &&  this.wiki.config.curatorUser
  &&  code == this.wiki.config.curatorCode.capitalize()
  ){
    user = this.wiki.config.curatorUser
    this.login_de&&bug( this, "configDefinedCurator:", user)
    this.loginLog( "using configured curator code, " + user)
    is_curator = true
    is_config_defined_curator = true
  }
  if( !user ){
    user = "SomeOne"
  ...
  if( (!is_guest && !is_config_defined_curator)
  && ( userpage.isDraft()
    || userpage.isNull()
  ...
  if( has_curator ){
    ...
    if( is_curator
    && that.wiki.config.curatorCode
    && that.wiki.config.curatorCode != SW.config.curatorCode.capitalize()
    && code != that.wiki.config.curatorCode.capitalize()
    ){
      that.de&&bug( "Curator code does not match config defined one")
      // ToDo: facebook
      if( true || !that.twitterName ){
        that.loginLog( "not the configured curator code")
        is_curator = false
      }else{
  ...
  if( false ){ // !is_guest || that.isGuestMember ){
    ...
    !codepage.isDisposableCode() && codepage.read( function(){
      ...
      || codepage.getFirstUser() != that.loginName
        ...
        // If wiki is very open because of default config, make sure user is a curator
        var codename = that.loginName
        if( that.isOwner ){
          codename = "Curator" + codename
