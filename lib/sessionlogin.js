...
// Return false if nothing happened, true if req is beeing processed.
  ...
  if( query && query.kudocracy ){
    var kudo_ctx;
    try{
      kudo_ctx = JSON.parse( decodeURIComponent( query.kudocracy ) );
      // The authentic login works for an hour
      // ToDo: should encrypt this somehow
      var age = Date.now - kudo_ctx.time;
      if( age > 3600 * 1000 ){
        kudo_ctx.authentic = false;
      }else{
        this.canScript = !!kudo_ctx.can_script;
      }
      // ToDo: use req.headers["referer"] and kudo_ctx.url to detect
      // requests coming from kudocracy? When not coming from kudocracy,
      // I could ignore the query parameter somehow.
    }catch(_){}
    ...
    var kudo_name = kudo_ctx.authentic && kudo_ctx.visitor.substring( 1 );
    // Login
    if( kudo_name ){
      new_name = kudo_name;
    }else{
